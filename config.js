...
if (businessMenuWrap && typeof CONFIG !== 'undefined' && CONFIG.businessServices) {
  ...
  Object.keys(groups).forEach(groupName => {
    ...
    groupWrap.innerHTML = `<p class="reveal font-mono text-xs uppercase tracking-[0.25em] text-bone/50 mb-4 pb-2 border-b border-line">${groupName}</p>`;
    const rowsWrap = document.createElement('div');
    rowsWrap.className = 'divide-y divide-line';
    groups[groupName].forEach(s => {
      const row = document.createElement('div');
      row.className = 'reveal flex items-center justify-between gap-4 py-5';
      row.innerHTML = `
        <div class="min-w-0">
